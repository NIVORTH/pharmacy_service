                      ...
                      <div class="container-fluid">
                        <div class="scrolling-wrapper row flex-row flex-nowrap">
                          <div class="scroll" v-for="service in filteredList"  >
                            <div class="card card-block pointer"  @click="ClickService(service.id)">
                        ...
                        <table class="table table-hover">
                          ...
                          <tbody>
                            ...
                            <tr v-for="service_detail  in service_details">
                              ...
                              <td>
                                 <input class="form-control" name="price" v-model="service_detail.price"
                                  @keyup="keyup_Cost(service_detail.price,service_detail.id)"
                                  @focus="handleFocus"
                                  @blur="handleBlur"
                                  type="number" />
                                <!-- {{currentUser.currency}}{{formatNumber(service_detail.price,2)}} -->
                              </td>
...
export default {
  ...
  computed: {
    ...
    filteredList() {
     ...
     sumOfTransactions() {
        return this.service_details.reduce((sum, transaction) => {
  ...
  methods: {
        ...
        //  this.details.push(response.data);
            let service_detail = response.data;
                service_detail['price'] = service_detail.price;
                  ...
                  let old_price = parseFloat(value.price);
                    this.service_price = parseFloat(old_price);
                  
                    this.sumOfTransactions = this.service_details.reduce((sum, transaction) => {
                        return sum += parseFloat(transaction.price);
                    }, 0);
                    if(this.details.length > 0){
                    
                      this.GrandTotal = this.sumOfTransactions + this.total_detail;
                      this.service_price = this.sumOfTransactions + this.total_detail;
                     
                      
                    }else{
                   
                      this.GrandTotal = this.sumOfTransactions;
                      this.service_price = this.sumOfTransactions;
    ...
    Calcul_Total() {
      ...
      const total_without_discount = parseFloat(
        this.total_detail - this.sale.discount
      );
      ...
      this.GrandTotal = parseFloat(
         total_without_discount + this.sale.TaxNet + this.sale.shipping + this.service_price
      );
    ...
    delete_Product_Detail(id) {
      for (var i = 0; i < this.details.length; i++) {
        if (id === this.details[i].detail_id) {
          let subtotal = this.details[i].subtotal;
