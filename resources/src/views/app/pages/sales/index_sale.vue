   ...
   <b-modal hide-footer size="sm" scrollable id="Show_invoice" :title="$t('Invoice_POS')">
      <div id="invoice-POS">
        <div style="max-width:500px;margin:0px auto">
            ...
            <div style="width: 32%; float: right; font-size: 12px; margin-top: -20px;">
                <span>Date: {{invoice_pos.sale.date}}​</span><br>
                <span>Tel: 070 611 644 / 095 956 868 </span><br>
                <!-- <span>Sale Status : {{invoice_pos.sale.statut}}</span><br>
                <span>Paymemt Status : {{invoice_pos.sale.payment_statut}}</span><br> -->
            </div>
            <div style="width: 63%; float: left; font-size: 12px; margin-top: -20px;">
              <span>Customer Name: {{invoice_pos.sale.client_name}}</span><br>
              <span>Tel: {{invoice_pos.sale.client_tele}}</span><br>
              <!-- <span>N˚: {{invoice_pos.sale.Ref}}</span><br> -->
              <!-- <span v-show="pos_settings.show_phone">{{$t('Phone')}} : {{invoice_pos.setting.CompanyPhone}}</span><br> -->
              <!-- <span>ABA : 000 336 487 Name : YAM KOMLINH</span><br>
              <span>ACLEDA : 095 956 868 Name : YAM KOMLINH</span> -->
            </div>
          <div>
            <table style="border-collapse: collapse;">
              <thead>
                <tr> 
                  <td style="width:40px; font-size: 14px; text-align:center; border: 1px solid;"><b>No</b></td>
                  <td style="width:270px; text-align:center; font-size: 14px; border: 1px solid;"><b>Description</b></td>
                  <td style="width:90px; text-align:center; font-size: 14px; border: 1px solid;"><b>Quantity</b></td>
                  <td style="width:110px; text-align:center; font-size: 14px; border: 1px solid;"><b>Unit Price</b></td>
                  <td style="width:110px; text-align:center; font-size: 14px; border: 1px solid;"><b>Subtotal</b></td>
                </tr>
                <!-- <tr>
                  <td colspan="3" style="border: 1px solid;">Service</td>
                </tr> -->
              </thead>
              <tbody>

                   <!--Service-->
                  <th colspan ="5" style = "border: 1px solid; text-align: left;" v-if="showDiv">
                    Service
                  </th>
                  <tr v-for="(service_detail, index) in service_details">
                 
                  <td style="width:40px; height:25px; font-size: 14px; text-align:center; border: 1px solid;">
                    <div id="app">
                        {{ index+1}}
                    </div>
                  </td>
                  
                  <td style="width:270px; font-size: 14px; text-align:left; border: 1px solid;" >
                    {{service_detail.name}}
                  </td>
                  <td style="width:90px; font-size: 14px; text-align:center; border: 1px solid;">
                    1
                  </td>
                  <td style="width:110px; font-size: 14px; text-align:right; border: 1px solid;">
                    {{currentUser.currency}}{{service_detail.price}}
                  </td>
                  <td style="width:110px; font-size: 14px; text-align:right; border: 1px solid;">
                    {{currentUser.currency}}{{formatNumber(parseFloat(service_detail.total),2)}}</td>
                </tr>

                <!--Products-->

                <th colspan ="5" style = "border: 1px solid; text-align: left;" v-if="showPro">
                  Product
                </th>
                ...
                <tr v-for="(detail_invoice, index) in invoice_pos.details">
                  ...
                  <td style="width:270px; font-size: 14px; text-align:left; border: 1px solid;" >
                    <span>{{detail_invoice.name}}
                    </span>
                  </td>
                  <td style="width:90px; font-size: 14px; text-align:center; border: 1px solid;">
                    {{formatNumber(detail_invoice.quantity,0)}}{{detail_invoice.unit_sale}}
                  </td>
                  <td style="width:110px; font-size: 14px; text-align:right; border: 1px solid;">
                    {{currentUser.currency}}{{formatNumber(detail_invoice.total/detail_invoice.quantity,2)}}
                  </td>
                  <td style="width:110px; font-size: 14px; text-align:right; border: 1px solid;">
                    {{currentUser.currency}}{{formatNumber(parseFloat(detail_invoice.total),2)}}</td>
                </tr>


                <!-- <tr style="margin-top:10px; font-size: 11px;">
                  <td colspan="3" class="total" style="text-align:right;">VAT 10%</td>
                  <td style="text-align:center;" class="total">{{currentUser.currency}}{{formatNumber(invoice_pos.sale.taxe)}}</td>
                </tr>
                <tr style="margin-top:10px; font-size: 11px;">
                  <td colspan="3" class="total" style="text-align:right;">{{$t('Discount')}}</td>
                  <td style="text-align:center;" class="total">{{currentUser.currency}}{{formatNumber(invoice_pos.sale.discount)}}</td>
                </tr> -->
                <tr style="margin-top:15px; font-size: 13px;">
                  <td colspan="2" class="total" style="text-align:left; padding-top: 5px"></td><br>
                  <td colspan="2" class="total" style="text-align:right; border: 1px solid;">{{$t('TotalAmount')}} (USD)</td>
                  <td style="text-align:right; border: 1px solid;" class="total">{{currentUser.currency}}{{formatNumber(parseFloat(invoice_pos.sale.GrandTotal),2)}}</td>
                </tr>

                <tr style="margin-top:15px; font-size: 13px;">
                  <td colspan="2" class="total" style="text-align:left;"></td>
                  <td colspan="2" class="total" style="text-align:right; border: 1px solid;">Total Amount (Riel)​ </td>
                  <td style="text-align:right; border: 1px solid;" class="total">៛{{formatNumber(invoice_pos.sale.GrandTotal * parseFloat(totalriel),2)}}</td>
                </tr>

                <!-- <tr v-show="invoice_pos.sale.paid_amount < invoice_pos.sale.GrandTotal" style="margin-top:15px; font-size: 12px;">
                  <td colspan="4" class="total" style="text-align:right;">{{$t('Paid')}} (USD)</td>
                  <td style="text-align:right;" class="total">
                    {{currentUser.currency}}{{formatNumber(parseFloat(invoice_pos.sale.paid_amount),2)}}</td>
                </tr> -->
                <tr v-show="invoice_pos.sale.paid_amount < invoice_pos.sale.GrandTotal" style="margin-top:15px; font-size: 13px;">
                  <td colspan="2" class="total" style="text-align:left;"></td> 
                  <td colspan="2" class="total" style="text-align:right; border: 1px solid;">{{$t('Balance')}} (USD)</td>
                  <td style="text-align:right; border: 1px solid;" class="total">
                    {{currentUser.currency}}{{formatNumber(parseFloat(invoice_pos.sale.GrandTotal - invoice_pos.sale.paid_amount),2)}}</td>
                </tr>
              </tbody>
            </table>
            <!-- <span style="font-size: 12px;">Today Rate: ៛​ {{ totalriel }}</span> -->
            <!-- <div style="font-size: 12px;">
                  <span>ABA : 000 336 487 Name : YAM KOMLINH</span>
                  <span>ACLEDA : 095 956 868 Name : YAM KOMLINH</span>
                </div> -->
          </div>
          <!-- <table class="change mt-3" style=" font-size: 10px;" v-show="invoice_pos.sale.paid_amount > 0">
            <thead>
              <tr style="background: #eee; ">
                ...
                <th style="text-align: left;" colspan="1">Cash​ Received:</th>
            ...
            <tbody>
              <tr v-for="payment_pos in payments">
                ...
                <td style="text-align: center;" colspan="1">{{currentUser.currency}}{{payment_pos.received_amount}}</td>
                <td style="text-align: center;" colspan="2">{{currentUser.currency}}{{formatNumber(payment_pos.montant)}}</td>
                <td style="text-align: right;" colspan="1">{{currentUser.currency}}{{formatNumber(payment_pos.change)}}</td>
              </tr>
            </tbody>
          </table> -->
          <!-- <br> -->
          <!-- <div class="info">
            <p style="font-size: 12px;">
              <span><b>Term & Condition</b></span><br>
              <span>{{reason}}​</span><br>
              <span>1. Purchased products cannot return.</span><br>
              <span>2. Payment term 1 week.</span><br>
              <span>3. Banking:</span><br>
              <span>ABA : 000671716 Name : HENG THANY</span> <br>
              <span>ACLEDA : 09000059770111 Name : YOEUN RITHY</span>
            </p>
          </div>
          <div>
            <div style="width: 25%; float: left; font-size: 12px;">
              <img src="/images/kh-qr.jpg" style="width: 100px; height: 100px">
            </div>
            <div style="width: 48%; float: left; font-size: 12px;">
              <img src="/images/riel.jpg" style="width: 100px; height: 100px">
            </div>
          </div>
          <br><br><br><br><br><br><br> -->
          <br>
          <!-- <div id="legalcopy" class="ml-2">
            <div style="width: 20%; float: left; font-size: 12px; text-align: center;" >
              <div>
                <span style="text-align: center;">Seller Signature</span><br><br><br>
                <span style="text-align: center;"><hr></span>
              </div>
            </div>
            <div style="width: 20%; float: right; font-size: 12px; text-align: center;">
              <span style="text-align: center;">Customer Signature</span><br><br><br>
