  ...
  methods: {
      ...
      if (this.adjustment.warehouse_id != "" && this.adjustment.warehouse_id != null) {
        this.timer = setTimeout(function () {
          var product_filter = _this.products.filter(function (product) {
            return product.name === _this.search_input || product.name.includes(_this.search_input);
          });
          if (product_filter.length === 1) {
            _this.SearchProduct(product_filter[0]);
          } else {
            _this.product_filter = _this.products.filter(function (product) {
              return product.name.toLowerCase().startsWith(_this.search_input.toLowerCase())
    ...
    Get_Product_Details: function Get_Product_Details(product_id) {
      ...
      axios.get("Products/" + product_id).then(function (response) {
        ...
        _this5.product.kh_name = response.data.kh_name;
        _this5.product.type = "add";
        if (_this5.product.product_variant_id) {
          axios.get("ProductVarians/" + _this5.product.product_variant_id).then(function (response) {
            _this5.product.unit = response.data.name;
            _this5.product.current = _this5.product.current / response.data.qty;
            _this5.add_product();
...
var render = function render() {
  ...
  return _c("div", {
    ...
    }, [_c("td", [_vm._v(_vm._s(detail.detail_id))]), _vm._v(" "), _c("td", [_vm._v(_vm._s(detail.name))]), _vm._v(" "), _c("td", [_c("span", {
