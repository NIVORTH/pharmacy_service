  ...
  methods: {
      ...
      if (this.purchase_return.warehouse_id != "" && this.purchase_return.warehouse_id != null) {
        this.timer = setTimeout(function () {
          var product_filter = _this3.products.filter(function (product) {
            return product.name === _this3.search_input || product.name.includes(_this3.search_input);
          });
          if (product_filter.length === 1) {
            _this3.SearchProduct(product_filter[0]);
          } else {
            _this3.product_filter = _this3.products.filter(function (product) {
              return product.name.toLowerCase().startsWith(_this3.search_input.toLowerCase())
    ...
    add_product: function add_product() {
      ...
      var value = this.details.find(function (details) {
        return details.product_id == _this5.product.product_id;
      });
      if (value) {
        this.makeToast("warning", this.$t("AlreadyAdd"), this.$t("Warning"));
      } else {
    ...
    Update_purchase_return: function Update_purchase_return() {
      ...
      if (this.verifiedForm()) {
        ...
        axios.put("returns/purchase/".concat(id), {
          ...
          nprogress__WEBPACK_IMPORTED_MODULE_1___default.a.done();
          _this6.makeToast("success", _this6.$t("Update.TitleReturn"), _this6.$t("Success"));
    ...
    Get_Product_Details: function Get_Product_Details(product_id) {
      ...
      axios.get("Products/" + product_id).then(function (response) {
        ...
        // this.add_product();
        // this.Calcul_Total();
        if (_this7.product.product_variant_id) {
          axios.get("ProductVarians/" + _this7.product.product_variant_id).then(function (response) {
            var New_Net_cost = Net_cost * response.data.qty;
            _this7.product.Net_cost = New_Net_cost ? New_Net_cost : Net_cost;
            _this7.product.product_qty = _this7.product.stock / response.data.qty;
            _this7.product.product_variant_name = response.data.name;
            _this7.add_product();
            _this7.Calcul_Total();
          });
        } else {
          var New_Net_cost = Net_cost;
          _this7.product.Net_cost = Net_cost;
          _this7.product.product_qty = _this7.product.stock;
          _this7.product.product_variant_name = response.data.unitSale;
          _this7.add_product();
          _this7.Calcul_Total();
        }
      });
    ...
    GetElements: function GetElements() {
      ...
      axios.get("returns/purchase/".concat(id, "/edit")).then(function (response) {
        _this8.purchase_return = response.data.purchase_return;
        ...
        _this8.Get_Products_By_Warehouse(_this8.purchase_return.warehouse_id);
        _this8.Calcul_Total();
...
var render = function render() {
  ...
  return _c("div", {
    ...
    }, [_vm._v(_vm._s(parseInt(detail.product_qty)) + " " + _vm._s(detail.product_variant_name))])]), _vm._v(" "), _c("td", [_c("div", {
