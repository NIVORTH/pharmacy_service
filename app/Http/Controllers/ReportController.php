...
class ReportController extends BaseController
{
    ...
    public function Sales_Client_Detail(request $request)
    {
        ...
        $sales = Sale::where('deleted_at', '=', null)->with(['client','details'])
            ->where(function ($query) use ($ShowRecord) {
                if (!$ShowRecord) {
                    return $query->where('user_id', '=', Auth::user()->id);
                }
        ...
        foreach ($sales as $sale) {
            if(!empty($sale['details'])){
                   foreach ($sale['details'] as $val) {
                        $item['date'] = $sale->date;
                        $item['name'] = $val['product']->name;
                        $item['price'] = $val->price;
                        $item['qty'] = $val->quantity;
                        $item['subtotal'] = $val->total;
                        $data[] = $item;
                   }
